...
internal unsafe class VkResourceSet : ResourceSet
{
        ...
        : base(ref description)
    {
        ...
        for (int i = 0; i < descriptorWriteCount; i++)
        {
            ...
            descriptorWrites[i].sType = VkStructureType.StructureTypeWriteDescriptorSet;
            ...
            if (type == VkDescriptorType.DescriptorTypeUniformBuffer ||
                type == VkDescriptorType.DescriptorTypeUniformBufferDynamic ||
                type == VkDescriptorType.DescriptorTypeStorageBuffer ||
                type == VkDescriptorType.DescriptorTypeStorageBufferDynamic)
            {
            ...
            else if (type == VkDescriptorType.DescriptorTypeSampledImage)
            {
                ...
                imageInfos[i].imageLayout = VkImageLayout.ImageLayoutShaderReadOnlyOptimal;
                descriptorWrites[i].pImageInfo = &imageInfos[i];
                SampledTextures.Add(Util.AssertSubtype<Texture, VkTexture>(texView.Target));
                RefCounts.Add(vkTexView.RefCount);
            }
            else if (type == VkDescriptorType.DescriptorTypeStorageImage)
            {
                var texView = Util.GetTextureView(this.gd, boundResources[i]);
                var vkTexView = Util.AssertSubtype<TextureView, VkTextureView>(texView);
                imageInfos[i].imageView = vkTexView.ImageView;
                imageInfos[i].imageLayout = VkImageLayout.ImageLayoutGeneral;
                descriptorWrites[i].pImageInfo = &imageInfos[i];
                StorageTextures.Add(Util.AssertSubtype<Texture, VkTexture>(texView.Target));
                RefCounts.Add(vkTexView.RefCount);
            }
            else if (type == VkDescriptorType.DescriptorTypeSampler)
            {
        ...
        Vk.UpdateDescriptorSets(this.gd.Device, descriptorWriteCount, descriptorWrites, 0, null);
    }
