...
///     <see cref="VKGraphics.Framebuffer" />, <see cref="Pipeline" />, vertex buffer, and index buffer must all be
...
public abstract class CommandList : IDeviceResource, IDisposable
{
    ...
    ///     When drawing, the active <see cref="Pipeline" /> must be compatible with the bound <see cref="VKGraphics.Framebuffer" />,
    ...
    /// <summary>
    ///     Sets the active <see cref="VKGraphics.Framebuffer" /> which will be rendered to.
    ///     When drawing, the active <see cref="VKGraphics.Framebuffer" /> must be compatible with the active <see cref="Pipeline" />.
    ///     A compatible <see cref="Pipeline" /> has the same number of output attachments with matching formats.
    /// </summary>
    /// <param name="fb">The new <see cref="VKGraphics.Framebuffer" />.</param>
    ...
    /// <summary>
    ///     Clears the color target at the given index of the active <see cref="VKGraphics.Framebuffer" />.
    ///     The index given must be less than the number of color attachments in the active <see cref="VKGraphics.Framebuffer" />.
    /// </summary>
    ...
    /// <summary>
    ///     Clears the depth-stencil target of the active <see cref="VKGraphics.Framebuffer" />.
    ///     The active <see cref="VKGraphics.Framebuffer" /> must have a depth attachment.
    ...
    /// <summary>
    ///     Sets all active viewports to cover the entire active <see cref="VKGraphics.Framebuffer" />.
    /// </summary>
    public void SetFullViewports()
    {
        SetViewport(0, new Viewport(0, 0, Framebuffer.Width, Framebuffer.Height, 0, 1));

        for (uint index = 1; index < Framebuffer.ColorTargets.Count; index++)
        {
    ...
    /// <summary>
    ///     Sets the active viewport at the given index to cover the entire active <see cref="VKGraphics.Framebuffer" />.
    /// </summary>
    /// <param name="index">The color target index.</param>
    ...
    /// <summary>
    ///     Sets all active scissor rectangles to cover the active <see cref="VKGraphics.Framebuffer" />.
    /// </summary>
    ...
    /// <summary>
    ///     Sets the active scissor rectangle at the given index to cover the active <see cref="VKGraphics.Framebuffer" />.
    /// </summary>
    /// <param name="index">The color target index.</param>
    ...
    /// <summary>
    ///     Performs API-specific handling of the <see cref="VKGraphics.Framebuffer" /> resource.
    /// </summary>
    /// <param name="fb"></param>
