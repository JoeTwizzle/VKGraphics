...
//namespace BrickEngine.Example.VoxelRenderer.Octree
//{
//    sealed class FastVoxelRendererOctree : IVolumeRenderer
//    {
...
//        public unsafe FastVoxelRendererOctree(GameWindow window, UnmanagedMemoryRange treeData, FilteredVolumeInfo info, UnmanagedMemoryRange voxelData)
//        {
//            Camera = new();
//            this.window = window;
...
//            var voxValueBuf = rf.CreateBuffer(new BufferDescription((uint)voxelData.Length, BufferUsage.StructuredBufferReadOnly));
//            gd.UpdateBuffer(voxValueBuf, 0, (IntPtr)voxelData.First, (uint)voxelData.Length / 2);
//            gd.UpdateBuffer(voxValueBuf, (uint)voxelData.Length / 2, (IntPtr)voxelData.First + (nint)voxelData.Length / 2, (uint)voxelData.Length / 2);

//            var octreeBuf = rf.CreateBuffer(new BufferDescription((uint)treeData.Length, BufferUsage.StructuredBufferReadOnly));
//            gd.UpdateBuffer(octreeBuf, 0, (IntPtr)treeData.First, (uint)treeData.Length);
...
//            voxelSet = rf.CreateResourceSet(new ResourceSetDescription(voxelLayout, voxGridInfo, octreeBuf, voxValueBuf));
...
//            //Create shader
//            var shaderResult = SpirvCompilation.CompileGlslToSpirv(File.ReadAllText("VolumeRenderer\\Octree\\FastVoxelOctree.comp"), "FastVoxelOctree.comp", ShaderStages.Compute, new GlslCompileOptions(window.GameSettings.Debug));
...
//            //Create Pipeline
//            raytracePipeline = rf.CreateComputePipeline(new(shader, [cameraBufferLayout, voxelLayout, renderBufferLayout], 8, 8, 1));
